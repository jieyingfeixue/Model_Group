...
const routes = [
  ...
  // 嵌套路由 — 所有业务页面包裹在 AppLayout 中（含侧边栏）
  {
    ...
    children: [
      ...
      { path: 'annotate', name: 'AnnotationHub', component: () => import('@/views/normal/AnnotationHub.vue') },
      { path: 'annotate/:taskId', name: 'AnnotationTool', component: () => import('@/views/normal/AnnotationHub.vue') },
