...
def list_public_datasets(
    ...
    datasets, total = Dataset.list_public(db, filters, page, size)
    ...
    for ds in datasets:
        ...
        items.append({
            ...
            "modality": (ds.filters or {}).get("modality", "unknown"),
            "sample_count": sum(counts.values()),
            "dataset_type": "official" if ds.review_status == "approved" else "community",
...
def get_dataset_detail(
    ...
    counts = DatasetItem.count_by_subset(db, dataset_id)
...
def preview_dataset_samples(
    ...
    total = query.count()
    items = (
        query.order_by(DatasetItem.item_id)
        .offset((page - 1) * size)
        .limit(size)
        .all()
    )
    ...
    samples = []
    for item in items:
        res = resource_map.get(item.resource_id)
        if res is None:
            continue
        ...
        subq = (
            ...
            .filter(Annotation.resource_id == item.resource_id)
        ...
        samples.append({
            ...
            "name": res.name,
            "file_url": get_file_url(res.file_path, expires=300),
            "subset": item.subset,
            "modality": res.modality,
            "meta_info": res.meta_info,
            "bboxes": latest_ann.bboxes if latest_ann else [],
            "review_status": latest_ann.review_status if latest_ann else "pending",
        })

    return {"total": total, "page": page, "size": size, "samples": samples}
...
def download_copy(
    ...
    流程：
    1. 读取原 Dataset + DatasetItem
    2. 新建 Dataset(owner_id=user_id)
    3. DatasetItem.bulk_insert() 批量复制条目
    4. 复制后与原数据集解耦（独立管理）

    API: POST /api/datasets/{id}/copy
    """
    from app.models.audit_log import AuditLog
    from app.models.dataset import Dataset
    from app.models.dataset_item import DatasetItem

    original = db.query(Dataset).filter(Dataset.dataset_id == dataset_id).first()
    if original is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="数据集不存在",
    ...
    if items:
        item_dicts = [
            {"resource_id": it.resource_id, "subset": it.subset}
            for it in items
        ]
        DatasetItem.bulk_insert(db, copy.dataset_id, item_dicts)
    ...
    AuditLog(
        ...
        action="copy_dataset",
        target_type="dataset",
        target_id=original.dataset_id,
        after_state={"copy_id": copy.dataset_id, "name": copy.name},
    ).save(db)
